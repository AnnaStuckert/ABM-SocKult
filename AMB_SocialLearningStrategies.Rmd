...
```{r}
library(pacman)
p_load(Momocs, ggplot2, pracma, tidyverse)
    ...
    if(df$ind_strategy[agent] == "social"){
      ...
      if(prevScore==15){
        ...
        if(strategy=="Best Member"){
          ...
          topscorerAgent <- prevRound %>% filter(score == max(score)) %>% df$id[1][1]
          currentScore <- getScore(guesses[agent,,round])
          if (currentScore < max(prevRound$score)){
            newGuess <- guesses[topscorerAgent, ,round-1]
          }
          else{
            stop <- 0
            k <- 1
        ...
        if(strategy=="Conformity"){
          ...
          #If yes, use that guess. 
          #If not, do individual learning (coded below)
          igotit <- as_tibble(guesses[, , round-1])
          iamtheman <- as_tibble(guesses[, , round-1]) %>% group_by_all() %>% count() %>% left_join(igotit)
          mostFreqGuess <- which.max(iamtheman$n)
          mostFreqGuessScore <- getScore(guesses[mostFreqGuess, ,round-1])
          if (prevScore < mostFreqGuessScore){
            newGuess <- guesses[mostFreqGuess, ,round-1]
          }
...
bmData <- rbind(bm0,bm10,bm20,bm30,bm40,bm50,bm60,bm70,bm80,bm90,bm100)
bmData$prop_ind <- as.factor(bmData$prop_ind)
write.csv(bmData,'bmData.csv')

#Combine Conformity data 
cfData <- rbind(cf0,cf10, cf20, cf30, cf40, cf50, cf60, cf70, cf80, cf90, cf100)
